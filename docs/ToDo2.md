...
Step 3: Add null‑move verification and safer conditions
Rationale: Unverified NMP fail‑high can prune tactical/mating lines. Verification limits false positives.

Changes (in NMP block of `search`):
- Disable NMP near mate bounds and in zugzwang‑prone endgames.
- On fail‑high at sufficient depth, run a verification re‑search at reduced depth before cutting.

Explanation: Only after a confirming search returns ≥ beta do we cut.

```cpp
// C++
// Before attempting NMP:
const bool nearMateWindow = (beta > VALUE_CHECKMATE_THRESHOLD - Value{64}) || (alpha < -VALUE_CHECKMATE_THRESHOLD + Value{64});
const bool zugProne = p.getMaterialNonPawn(us) == 0; // customize further if you track endgame phases

if (SearchConfig::USE_NMP) {
  if (doNull && !isPv && depth >= SearchConfig::NMP_DEPTH && !hasCheck && !zugProne && !nearMateWindow) {
    Depth r = SearchConfig::NMP_REDUCTION + (depth > 8 || (depth > 6 && p.getGamePhase() >= 3));
    Depth newDepth = std::max<Depth>(DEPTH_NONE, depth - r - 1);
    ...
    if (nValue >= beta) {
      ...
      if (depth >= 6) {
        Depth verifyDepth = std::max<Depth>(DEPTH_NONE, depth - r - 2);
...
Prompt for step 3:
- Please harden NMP in 'src/engine/Search.cpp': disable it when near mate bounds or in zugzwang‑prone endgames, and add a verification re‑search on fail‑high at depth ≥ 6 before cutting, as in the snippet. I will build and test locally and report back.
