...
setarch $(uname -m) -R ./cmake-build-wsl-debug-tsan/test/FrankyCPP_v1.8_Test --gtest_filter=-*SpeedTests.*:-*TimingTests.*

# Option 2: Disable ASLR system-wide (revert with value 2)
sudo sysctl -w kernel.randomize_va_space=0
./cmake-build-wsl-debug-tsan/test/FrankyCPP_v1.8_Test --gtest_filter=-*SpeedTests.*:-*TimingTests.*
sudo sysctl -w kernel.randomize_va_space=2  # Re-enable after testing

# Run specific tests (e.g., SMP tests only)
setarch $(uname -m) -R ./cmake-build-wsl-debug-tsan/test/FrankyCPP_v1.8_Test --gtest_filter=*SMP*
```
...
.\cmake-build-win-debug-asan\test\FrankyCPP_v1.8_Test.exe

.\cmake-build-win-relwithdebinfo-asan\test\FrankyCPP_v1.8_Test.exe --gtest_filter=-*SpeedTests.*:-*TimingTests.*
```
...
./cmake-build-wsl-debug-asan/test/FrankyCPP_v1.8_Test
```
...
Then run:
```bash
TSAN_OPTIONS="suppressions=tsan_suppressions.txt" ./cmake-build-wsl-debug-tsan/test/FrankyCPP_v1.8_Test
```
...
2. **Run problematic test repeatedly**:
   ...
   for i in {1..10}; do
     echo "Run $i"
     ./cmake-build-wsl-debug-tsan/test/FrankyCPP_v1.8_Test --gtest_filter=SearchSmpTest.* 2>&1 | tee run_$i.log
   done
   ```
