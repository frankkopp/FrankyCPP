...
Higher‑precision timer tail
Rationale: reduce overshoot on Windows by busy‑waiting the last few milliseconds instead of sleeping.
Prompt: Please modify startTimer() in src/engine/Search.cpp to switch from sleep to a short busy‑wait for the final ~2–3ms before deadline. I will build and run.
